...
import { Component, OnInit, Input, Output, EventEmitter, ElementRef, ViewChild, forwardRef, style, state, ViewEncapsulation, Renderer2 } from '@angular/core';
...
export class BroadcastCalendar implements OnInit, ControlValueAccessor {
  ...
  @Input() previousDateLimit: string;
  ...
  ngAfterViewInit() {   
     if(this.previousDateLimit){
       this.prevDateValue = new Date(this.previousDateLimit);
     }else if(this.minDate){
       let minD = new Date(this.minDate);
       this.prevDateValue =  new Date(minD.getFullYear() - 2, minD.getMonth(), minD.getDate());
     }else{
       let curD = new Date();
       this.prevDateValue = new Date(curD.getFullYear() - 2, curD.getMonth(), curD.getDate());
     }
     //to keep track if maxdate is assigned to calender
     if(this.maxDate){
  ...
  generateYearArray() {
    ...
    let cDate = new Date(this.minDate);  
    ...
    for (let i = this.startMonth; i <= this.endMonth; i++) {
      ...
      if(i == this.endMonth){ 
        //Saving the initial end date to set the start month when user clicks on next year button      
        this.initialEndDate = new Date(this.year, k, endDate);
      }
  ...
  onDateSelect(evt) {
    ...
    this.filterAttribute.hasSelection = (selectedDate.length > 0) ? true : false;
    this.changeInputBkgroundColor();
    ...
    if (id && id !== '') {
      ...
      if (this.isSingleSelect) {
        if (this.selectedDates.length > 0) {
          if(this.showAsRange){
          	if (this.selectedDates[0].indexOf('-') < 0) {
            ...
            this.deselectRangeDateForSingleSelect(new Date(this.selectedDates[0]), new Date(this.selectedDates[0]));
            this.selectedDates.pop();
            this.selectedDates.push(selectedDate.toString());
            this.changeTheColorOfTheCellOnSelect(ele);
          }
  ...
  getCellStyle(year, month, date, ri) {
    if (this.minDate && this.maxDate) {
      ...
      if (this.isSingleSelect) {
        let minProjdate;
        if(this.minDate){
          minProjdate = this.minDate;
        }else{
          minProjdate = new Date();
          minProjdate.setDate(new Date(minProjdate).getDate() - 1);
        }       
       /* if (new Date(minProjdate).getTime() > d.getTime() || new Date(this.maxDate).getTime() < d.getTime()) {
          return "lightgray";
        }*/
         /*since we are enabling the selection of previous dates , so comparing time with previous  
         time instead of minDate */
        if(this.prevDateValue.getTime() > d.getTime() || new Date(this.maxDate).getTime() < d.getTime()){
          return "lightgray";
        }
      } else {
  ...
  onShowCalendarClick() {
    ...
    if(this.filterAttribute){
      this.minDate = this.filterAttribute.minDate;
      this.maxDate = this.filterAttribute.maxDate;
    }
  ...
  onFocusOut(evt) {
    ...
    if (evt.target.value != '') {
      if( this.isDateValid(evt.target.value) ) {
  ...
  nextYear() {     
    if (this.startOfTheMonth === '') {  
      if(!this.initialEndDate) this.startOfTheMonth = this.maxDate;  
      this.initialEndDate.setMonth(this.initialEndDate.getMonth() + 1);
      this.startOfTheMonth = this.initialEndDate.toString();
    ...
    //Donot change this.maxDate if maxDateSet is set to true
    if (this.isSingleSelect && !this.maxDateSet) {
  ...
  prevYear() {
    ...
    if (this.startOfTheMonth === '') {
     ...
     if(this.isSingleSelect){
        if( this.prevDateValue.getTime() <= new Date(this.startOfTheMonth).getTime())
  ...
  onCellHover(evt) {
    if (this.isSingleSelect && this.selectedDates.length === 1 && this.showAsRange && this.selectedDates[0].indexOf('-') < 0) {
  ...
  changeInputBkgroundColor() {
    let bgColor = this.filterAttribute.disabled ? '#eeeeee' : this.inputValue !== '' ? 'aliceblue' : '#ffffff';
    this.renderer.setStyle(this.dateBox.nativeElement,'background-color',bgColor);
  }
  ...
  isDateValid(p_value) {
    ...
    const dateRange:string[] = p_value.split('-');

    let firstDate = moment(dateRange[0], DATE_FORMAT, true).isValid();
    let lastDate = true;
 
    if ( dateRange[1] ) {
      lastDate = moment(dateRange[1], DATE_FORMAT, true).isValid();
    }
